...
def get_name():
    name = input(slowth('"What is your name, wanderer?"')).strip().title()
    return name


def greet(name):
    greeting = input(slowth('"Well met, {name}. We rarely see strangers here. Where are you from?"'.format(name=name))).strip().title()
    return greeting
...
def offer_quest(name, homeland):
    ...
    if answer.lower() != 'n' and answer.lower() != 'no':
